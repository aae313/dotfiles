...
if $nu.is-interactive and ($env.ZELLIJ? | is-empty) {
  let zellij_clients = (^zellij --session main action list-clients | complete)
  let main_has_client = (
    $zellij_clients.stdout
    | lines
    | skip 1
    | any {|line| $line | str trim | is-not-empty }
  )

  if not $main_has_client {
...
$env.LS_COLORS = (
  open --raw ($nu.config-path | path dirname | path join ls_colors.txt)
  | str trim
)
...
$env.FZF_DEFAULT_OPTS = "--highlight-line --cycle --layout=reverse --height=80% --bind=tab:down,btab:up,ctrl-space:toggle"
